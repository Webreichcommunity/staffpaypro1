...
export const Button = ({ children, tone = "orange", className = "", loading, type = "button", ...props }) => (
  <button
    type={type}
    className={`inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-center text-sm font-semibold shadow-sm outline-none transition focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60 ${toneMap[tone]} ${className}`}
...
export const Input = ({ label, className = "", ...props }) => (
  <label className="grid gap-1.5 text-sm font-medium text-gray-700">
    ...
    <input
      className={`min-h-10 rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-orange-500 focus:ring-2 focus:ring-orange-100 ${className}`}
      {...props}
    />
  </label>
);
...
export const Select = ({ label, children, className = "", ...props }) => (
  <label className="grid gap-1.5 text-sm font-medium text-gray-700">
    {label && <span>{label}</span>}
    <select
      className={`min-h-10 rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 outline-none transition focus:border-orange-500 focus:ring-2 focus:ring-orange-100 ${className}`}
      {...props}
    >
      {children}
    </select>
...
export const Textarea = ({ label, className = "", ...props }) => (
  <label className="grid gap-1.5 text-sm font-medium text-gray-700">
    {label && <span>{label}</span>}
    <textarea
      className={`min-h-20 rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-orange-500 focus:ring-2 focus:ring-orange-100 ${className}`}
      {...props}
    />
  </label>
);
...
export const Card = ({ children, className = "" }) => (
  <section className={`rounded-xl border border-gray-200 bg-white p-3.5 shadow-sm sm:p-4 ${className}`}>{children}</section>
);
...
export const Modal = ({ title, children, onClose }) => (
  <div className="fixed inset-0 z-50 grid items-end bg-gray-950/50 sm:place-items-center sm:p-4">
    <div className="max-h-[92vh] w-full max-w-2xl overflow-auto rounded-t-2xl bg-white p-4 shadow-xl sm:rounded-xl">
        ...
        <tbody className="divide-y divide-gray-100">
          {rows.length ? rows.map((row) => (
            <tr key={row.id || JSON.stringify(row)} className="align-top hover:bg-orange-50/40">
              {columns.map((column) => <td key={column.key} className="max-w-64 px-3 py-2.5 text-gray-700 [&_.flex]:flex-nowrap">{column.render ? column.render(row) : row[column.key]}</td>)}
            </tr>
          )) : (
            <tr>
...
export const SearchBox = ({ value, onChange, placeholder = "Search" }) => (
  <div className="relative">
    ...
    <input
      className="min-h-10 w-full rounded-lg border border-gray-200 bg-white py-2 pl-10 pr-3 text-sm outline-none transition focus:border-orange-500 focus:ring-2 focus:ring-orange-100"
      value={value}
